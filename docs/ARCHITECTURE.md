...
- **Observers**: Observer notifications (metrics, turn tracking) run asynchronously so the processor chain is not blocked; data frames still flow in order. Observers are documented as safe for concurrent invocation (e.g. metrics use a mutex).
